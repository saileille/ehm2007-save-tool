...
impl Player {
    pub fn add_column(&mut self, data: &Data, counter: usize, header: &str, s: &Staff, p: &data::player::Player) {
        self.columns.push(match header {
            ...
            "GK Rating" => json!(s.relative_gk_rating(data)),
            "LD Rating" => json!(s.relative_ld_rating(data)),
            "RD Rating" => json!(s.relative_rd_rating(data)),
            "LW Rating" => json!(s.relative_lw_rating(data)),
            "C Rating" => json!(s.relative_c_rating(data)),
            "RW Rating" => json!(s.relative_rw_rating(data)),
